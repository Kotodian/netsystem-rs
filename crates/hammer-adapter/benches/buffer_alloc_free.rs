...
use hammer_adapter::{BufferPool, DataPlaneRuntime};
...
fn bench_alloc_free_single(c: &mut Criterion) {
    ...
    group.bench_function("empty", |b| {
        b.iter_batched(
            || BufferPool::with_capacity(2048, 4096),
            |pool| {
...
fn bench_alloc_free_batch256(c: &mut Criterion) {
    ...
    for &batch in &[64usize, 256, 1024] {
        group.bench_with_input(BenchmarkId::new("empty", batch), &batch, |b, &batch| {
            b.iter_batched(
                || BufferPool::with_capacity(2048, batch.max(4096)),
                |pool| {
...
fn bench_runtime_alloc_free(c: &mut Criterion) {
    ...
    group.bench_function("single", |b| {
        b.iter_batched(
            || DataPlaneRuntime::with_capacities(2048, 4096, 256, 64),
            |runtime| {
    ...
    group.bench_function("batch_256", |b| {
        b.iter_batched(
            || DataPlaneRuntime::with_capacities(2048, 4096, 256, 64),
            |runtime| {
                let mut indices = std::vec::Vec::with_capacity(256);
                for _ in 0..256 {
