...
mod tests {
    ...
    fn ack(now: Instant, bytes: u32) -> AckedPacket {
        AckedPacket {
            packet_number: 1,
            bytes,
            sent_at: now,
    ...
    #[test]
    fn cubic_slow_start_grows_by_acknowledged_bytes() {
        let now = Instant::now();
        let mut cubic = CubicController::new(1_000);
        ...
        assert_eq!(cubic.congestion_window(), 11_000);
        assert_eq!(cubic.delivered(), 1_000);
        assert_eq!(cubic.min_rtt(), Some(Duration::from_millis(10)));
    }

    #[test]
    fn cubic_loss_reduces_window_and_ends_slow_start() {
        let now = Instant::now();
        let mut cubic = CubicController::new(1_000);
        ...
        assert_eq!(cubic.congestion_window(), 7_000);
        assert_eq!(cubic.slow_start_threshold, 7_000);
        assert!(cubic.epoch_start.is_none());
    }

    #[test]
    fn cubic_mtu_update_preserves_minimum_window() {
        let mut cubic = CubicController::new(1_000);
        cubic.congestion_window = 2_000;

        cubic.on_mtu_update(1_500);

        assert_eq!(cubic.max_datagram_size(), 1_500);
        assert_eq!(cubic.congestion_window(), 3_000);
    }
}
