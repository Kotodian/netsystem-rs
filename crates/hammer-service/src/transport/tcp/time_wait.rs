...
pub struct TcpTimeWaitNode<C: CongestionController + 'static> {
    session_queue: TcpQueueHandle<C>,
}

impl<C> Node for TcpTimeWaitNode<C>
where
...
where
    C: CongestionController + 'static,
{
    let next = TcpTimeWaitNode::<C>::runtime_nexts(runtime)?;
    tcp_time_wait_frame::<C>(runtime, frame, TcpQueueHandle::new(data), next)
}

fn tcp_time_wait_frame<C>(
    runtime: &DataPlaneRuntime,
    frame: &mut BufferFrame,
    session_queue: TcpQueueHandle<C>,
...
where
    C: CongestionController + 'static,
{
    let tcp_output = next[TcpTimeWaitNext::Output as usize];
    ...
    frame.rewrite_indices_batched(runtime.preferred_frame_batch_width(), |index| {
        match tcp_time_wait_index(runtime, index, session_queue, tcp_output, &mut next_frames) {
            Ok(()) => Ok(None),
            Err(_) => {
    ...
    C: CongestionController + 'static,
{
    ...
    let result = {
        ...
        let state = queue
            ...
            .ok_or_else(|| CoreError::internal("tcp time-wait session is missing"))?
            .clone();
        let connection: TcpConnection<TimeWait, _> = state.try_into()?;
        let (next_state, control) = connection.receive_time_wait(&packet)?;
        let state = queue
