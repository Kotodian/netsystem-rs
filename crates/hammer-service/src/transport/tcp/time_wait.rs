    ...
    C: CongestionController + 'static,
{
    ...
    tcp_time_wait_frame::<C>(runtime, frame, Some(TcpSessionQueueHandle::new(data)), next)
}
    ...
    C: CongestionController + 'static,
{
    ...
    let result = TcpSessionProtocol::with_queue(session_queue, |queue: &mut super::session::TcpSessionQueue<C>| {
        ...
        let connection: TcpConnection<TimeWait, _> = queue.take_connection(session_id)?;
