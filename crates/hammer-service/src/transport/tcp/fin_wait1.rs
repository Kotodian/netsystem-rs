...
pub struct TcpFinWait1Node {
    #[node(default)]
    session_queue: Option<SessionQueueHandle>,
}

impl TcpFinWait1Node {
    #[inline]
...
fn tcp_fin_wait1_process(
    runtime: &DataPlaneRuntime,
    data: NodeRuntimeData,
    frame: &mut BufferFrame,
) -> CoreResult<NodeResult> {
    let next = TcpFinWait1Node::runtime_nexts(runtime)?;
    tcp_fin_wait1_frame(runtime, frame, Some(SessionQueueHandle::new(data)), next)
}

fn tcp_fin_wait1_frame(
    runtime: &DataPlaneRuntime,
    frame: &mut BufferFrame,
    session_queue: Option<SessionQueueHandle>,
    ...
    let session_queue = session_queue
        .ok_or_else(|| CoreError::internal("tcp fin-wait1 node missing session queue"))?;
    let congestion = next[TcpFinWait1Next::Congestion as usize];
    ...
    frame.rewrite_indices_batched(runtime.preferred_frame_batch_width(), |index| {
        match tcp_fin_wait1_index(runtime, index, session_queue, congestion, &mut next_frames) {
            Ok(()) => Ok(None),
            Err(_) => {
...
fn tcp_fin_wait1_index(
    ...
    let result = TcpSessionProtocol::with_queue(session_queue, |queue: &mut TcpSessionQueue| {
        ...
        let connection: TcpConnection<FinWait1, TcpServiceController> =
