...
use super::state_machine::FinWait1;

#[hammer_component_macros::node_next]
pub enum TcpFinWait1Next {
    Output,
    Drop,
}

#[hammer_component_macros::node(role = internal, next = TcpFinWait1Next)]
pub struct TcpFinWait1Node<C: CongestionController + 'static> {
    #[node(default)]
    session_queue: Option<TcpSessionQueueHandle<C>>,
}

impl<C> TcpFinWait1Node<C>
where
...
impl<C> Node for TcpFinWait1Node<C>
where
    C: CongestionController + 'static,
{
    #[inline(always)]
    fn process(
        ...
        tcp_fin_wait1_frame(runtime, frame, self.session_queue, next)
    }

    #[inline]
    fn node_process(&self) -> NodeProcessFn {
        tcp_fin_wait1_process::<C>
    }
    ...
    fn node_runtime_data(&self) -> CoreResult<NodeRuntimeData> {
        self.session_queue
            ...
            .ok_or_else(|| CoreError::internal("tcp fin-wait1 node missing session queue"))
    }
}

fn tcp_fin_wait1_process<C>(
    runtime: &DataPlaneRuntime,
...
where
    C: CongestionController + 'static,
{
    let next = TcpFinWait1Node::<C>::runtime_nexts(runtime)?;
    tcp_fin_wait1_frame::<C>(runtime, frame, Some(TcpSessionQueueHandle::new(data)), next)
}

fn tcp_fin_wait1_frame<C>(
    runtime: &DataPlaneRuntime,
    frame: &mut BufferFrame,
    session_queue: Option<TcpSessionQueueHandle<C>>,
    next: [NodeId; TcpFinWait1Next::COUNT],
) -> CoreResult<NodeResult>
where
    C: CongestionController + 'static,
{
    let session_queue = session_queue
        .ok_or_else(|| CoreError::internal("tcp fin-wait1 node missing session queue"))?;
    let tcp_output = next[TcpFinWait1Next::Output as usize];
    let drop_next = next[TcpFinWait1Next::Drop as usize];
    let mut next_frames = NodeNextFrames::default();
    frame.rewrite_indices_batched(runtime.preferred_frame_batch_width(), |index| {
        match tcp_fin_wait1_index(runtime, index, session_queue, tcp_output, &mut next_frames) {
            Ok(()) => Ok(None),
            Err(_) => {
    ...
    C: CongestionController + 'static,
{
    ...
    let result = TcpSessionProtocol::with_queue(session_queue, |queue: &mut super::session::TcpSessionQueue<C>| {
        let (session_id, _, _) = queue
            ...
            .ok_or_else(|| CoreError::internal("tcp fin-wait1 session is missing"))?;
        let connection: TcpConnection<FinWait1, _> = queue.take_connection(session_id)?;
