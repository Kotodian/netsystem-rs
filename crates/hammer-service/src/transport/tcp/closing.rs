    ...
    C: CongestionController + 'static,
{
    ...
    fn process(
        ...
        tcp_closing_frame(runtime, frame, self.session_queue, next)
    }
    ...
    C: CongestionController + 'static,
{
    ...
    tcp_closing_frame::<C>(runtime, frame, TcpQueueHandle::new(data), next)
}
    ...
    C: CongestionController + 'static,
{
    ...
    frame.rewrite_indices_batched(runtime.preferred_frame_batch_width(), |index| {
        match tcp_closing_index(runtime, index, session_queue, tcp_output, &mut next_frames) {
            Ok(()) => Ok(None),
            Err(_) => {
    ...
    C: CongestionController + 'static,
{
    ...
    let result = {
        ...
        let state = queue
            ...
            .ok_or_else(|| CoreError::internal("tcp closing session is missing"))?
            .clone();
        let connection: TcpConnection<Closing, _> = state.try_into()?;
        let (next_state, control) =
            connection.receive_closing(&mut queue, session_id, &packet)?;
        let state = queue
