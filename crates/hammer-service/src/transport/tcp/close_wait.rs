    ...
    C: CongestionController + 'static,
{
    ...
    tcp_close_wait_frame::<C>(runtime, frame, TcpQueueHandle::new(data), next)
}
    ...
    C: CongestionController + 'static,
{
    ...
    let result = {
        ...
        let state = queue
            ...
            .ok_or_else(|| CoreError::internal("tcp close-wait session is missing"))?
            .clone();
        let connection: TcpConnection<CloseWait, _> = state.try_into()?;
        let (next_state, control) =
            connection.receive_close_wait(&mut queue, session_id, &packet)?;
        let state = queue
