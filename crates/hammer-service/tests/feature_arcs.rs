...
fn feature_config_lifecycle_preserves_arc_contracts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let second_hardware =
        interfaces.register_interface(&mut runtime, device_class_index, 2, hw_class_index, 0);
    ...
    let replacement_hardware =
        interfaces.register_interface(&mut runtime, device_class_index, 3, hw_class_index, 0);
    ...
    let final_hardware =
        interfaces.register_interface(&mut runtime, device_class_index, 4, hw_class_index, 0);
