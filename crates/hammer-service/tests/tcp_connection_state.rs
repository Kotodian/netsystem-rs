...
use hammer_service::transport::tcp::{
    DEFAULT_TCP_OUTPUT_PAYLOAD_LEN, TcpConnectionTimerKind, TcpInputNext,
};
...
fn tcp_connection_does_not_keep_private_pacing_deadline() {
    let source = fs::read_to_string(
        Path::new(env!("CARGO_MANIFEST_DIR")).join("src/transport/tcp/connection.rs"),
    )
    .expect("read tcp connection");
