...
use hammer_service::transport::tcp::{
    TcpLookupKind, TcpV4ConnectionKey, TcpV4ListenerKey, TcpV6ConnectionKey, TcpV6ListenerKey,
    TcpWorkerOwnedState,
};
...
fn tcp_lookup_prefers_established_connection_before_listener() {
    ...
    let lookup = snapshot
        .lookup_v4(connection_key, listener_key)
...
fn tcp_lookup_listener_snapshot_returns_owner_worker_and_kind_deterministically() {
    ...
    let snapshot = owner.publish_snapshot();
    let lookup = snapshot
        .lookup_v6(miss_connection_key, listener_key)
