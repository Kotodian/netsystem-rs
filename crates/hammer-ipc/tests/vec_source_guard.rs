//! Source guard: production hammer-ipc modules must not allocate via `std::vec`.
...
#[test]
fn production_ipc_src_avoids_std_vec() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src");
    let mut violations = Vec::new();
    collect_violations(&root, &mut violations);
    assert!(
        violations.is_empty(),
        "production hammer-ipc src must use hammer_infra::vec; found:\n{}",
        violations.join("\n")
    );
}
