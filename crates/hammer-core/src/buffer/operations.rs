use super::{BufferFlags, BufferMain, BufferThreadCache, RefMut};
...
impl BufferMain {
    ...
    pub fn alloc_from_pool(
        ...
        self.pools[usize::from(pool_index)]
            .alloc_indices(&mut caches[usize::from(pool_index)], indices)
    }
    ...
    fn chain_last(&self, first: u32, exclusive: bool) -> u32 {
        ...
        for _ in 0..capacity {
            ...
            let buffer = unsafe { self.buffer_unchecked(current) };
            if exclusive {
    ...
    #[doc(hidden)]
    pub fn chain_init(&self, caches: &mut [RefMut<'_, BufferThreadCache>], first: u32) {
        self.validate_caches(caches);
        // SAFETY: the caller owns the exclusive chain head.
        let buffer = unsafe { self.buffer_mut_unchecked(first) };
        assert!(
    ...
    pub fn chain_link(
        ...
        unsafe {
            let buffer = self.buffer_mut_unchecked(next);
            buffer.cacheline0.current_length = 0;
            buffer.set_next_buffer(None);
            self.buffer_mut_unchecked(last).set_next_buffer(Some(next));
        }
        next
    }
    ...
    fn append_last(&self, first: u32, last: u32, data: &[u8]) -> usize {
        ...
        let tail = unsafe { self.buffer_unchecked(last) };
        let count = data
            .len()
        ...
        let total = if first != last {
            ...
            (unsafe { self.buffer_unchecked(first) }.total_length_not_including_first as usize)
                .checked_add(count)
        ...
        unsafe {
            self.buffer_mut_unchecked(last)
                .put_uninit(count as u16)
                .copy_from_slice(&data[..count]);
            if first != last {
                self.buffer_mut_unchecked(first)
                    .total_length_not_including_first = total as u32;
            }
        }
        count
    }
    ...
    pub fn chain_append_with_alloc(
        ...
        self.append_from_pool(caches, first, last, data, pool_index)
    }
    ...
    fn append_from_pool(
        ...
        while copied < data.len() {
            ...
            let mut next = [0];
            if self.alloc_from_pool(caches, &mut next, pool_index) == 0 {
                break;
            }
            *last = self.chain_link(caches, *last, next[0]);
        }
        copied
    }
    ...
    pub fn add_data(
        ...
        if *head == u32::MAX
            && self.alloc_from_pool(caches, core::slice::from_mut(head), pool_index) == 0
        {
            return 0;
        }
        ...
        unsafe { self.buffer_mut_unchecked(*head) }
            .cacheline0
            .flags
        ...
        loop {
            ...
            let buffer = unsafe { self.buffer_mut_unchecked(last) };
    ...
    #[doc(hidden)]
    pub fn attach_clone(&self, caches: &mut [RefMut<'_, BufferThreadCache>], head: u32, tail: u32) {
        self.validate_caches(caches);
        ...
        while let Some(index) = current {
            ...
            let buffer = unsafe { self.buffer_unchecked(index) };
            assert!(
        ...
        let buffer = unsafe { self.buffer_unchecked(tail) };
        let total = buffer
            .current_len()
        ...
        let head = unsafe { self.buffer_mut_unchecked(head) };
...
pub(super) mod tests {
    ...
    pub(in crate::buffer) fn allocation_chains_and_reference_release() {
        ...
        let mut tail = [0];
        assert_eq!(main.alloc_from_pool(&mut caches, &mut tail, 1), 1);
        main.chain_init(&mut caches, heads[0]);
        assert_eq!(
            main.chain_append(&mut caches, heads[0], heads[0], &[1, 2, 3, 4]),
            4
        );
        assert_eq!(main.chain_link(&mut caches, heads[0], tail[0]), tail[0]);
        assert_eq!(
            main.chain_append(&mut caches, heads[0], tail[0], &[5, 6, 7, 8]),
            4
        );
        ...
        assert_eq!(main.add_data(&mut caches, 0, &mut heads[0], &[]), 0);
        assert_eq!(main.chain_append(&mut caches, heads[0], tail[0], &[9]), 1);
        // Ownership: this test retains the exclusive chain during inspection.
        {
            let buffer = main.buffer(&caches, heads[0]);
            assert_eq!(buffer.total_length_not_including_first, 5);
            assert!(!buffer.flags().contains(BufferFlags::TOTAL_LENGTH_VALID));
        }
        assert!(
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                main.attach_clone(&mut caches, heads[1], tail[0]);
            }))
            .is_err()
        );
        // Ownership: this test exclusively owns heads[0]; manufacture the exact
        // one-byte counter boundary to verify rejection before head publication.
        main.buffer(&caches, heads[0])
            .cacheline0
            .ref_count
            .store(u8::MAX, Ordering::Relaxed);
        assert!(
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                main.attach_clone(&mut caches, heads[1], heads[0]);
            }))
            .is_err()
        );
        // Ownership: the rejected attachment retained both original obligations.
        main.buffer(&caches, heads[0])
            .cacheline0
            .ref_count
        ...
        assert!(
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                // Ownership: attempt only the checked borrow boundary; it must reject
                // this shared segment before returning any mutable reference.
                main.buffer_mut(&mut caches, heads[0]);
            }))
            .is_err()
        );
        // Ownership: the test owns both head obligations; these temporary borrows
        // finish before either release and no shared tail mutation occurs.
        {
            assert_eq!(main.buffer(&caches, heads[0]).ref_count(), 2);
            assert_eq!(main.buffer(&caches, tail[0]).ref_count(), 2);
            assert_eq!(
                main.buffer(&caches, heads[1])
                    .total_length_not_including_first,
                9
            );
            assert!(
                !main
                    .buffer(&caches, heads[1])
                    .flags()
        ...
        #[cfg(debug_assertions)]
        {
            ...
            assert_eq!(main.cached_free_buffers(&caches, 1), cached_free);
        }

        let capacity = main.pools[0].buffer_count;
        let mut retained = vec![u32::MAX; capacity + 1];
        assert_eq!(
            main.alloc_from_pool(&mut caches, &mut retained, 0),
            capacity
        );
        ...
        assert_eq!(main.add_data(&mut caches, 0, &mut head, &data), data_size);
        assert_ne!(head, u32::MAX);
        // Ownership: add_data published its partial chain into head even though
        // the next allocation failed; the test retains it until explicit free.
        {
            assert_eq!(main.buffer(&caches, head).current(), &data[..data_size]);
            assert!(
                !main
                    .buffer(&caches, head)
                    .flags()
