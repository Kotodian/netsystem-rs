...
/// Counters partitioned by the `network` label (tcp / udp). ICMP is
/// intentionally ignored by the metrics surface for now.
#[derive(Clone)]
...
impl NetworkCounters {
    /// Counters whose entries carry only the `network` label.
    pub fn new(scope: &MetricsScope, name: &str) -> Self {
        Self::with_labels(scope, name, &[])
    }

    /// Counters whose entries also carry the supplied extra labels in
    /// addition to `network=tcp|udp`.
    pub fn with_labels(scope: &MetricsScope, name: &str, extra: &[(String, String)]) -> Self {
        Self {
            tcp: counter_for_network(scope, name, extra, "tcp"),
            udp: counter_for_network(scope, name, extra, "udp"),
        }
    }

    pub fn inc(&self, network: Network) {
        match network {
            Network::Tcp => self.tcp.inc(),
            Network::Udp => self.udp.inc(),
            Network::Icmp => {}
        }
    }
}

fn counter_for_network(
    scope: &MetricsScope,
    name: &str,
    extra: &[(String, String)],
    network: &str,
...
mod tests {
    ...
    fn counters_reuse_the_same_registered_metric() {
        ...
        assert_eq!(samples[0].labels[0].key, "network");
        assert_eq!(samples[0].labels[0].value, "tcp");
    }

    #[test]
    fn labels_are_order_insensitive() {
        let registry = MetricsRegistry::new();
        let scope = registry.scope("route", "router", "default");

        scope
            .counter_with_labels("decision_total", [("network", "tcp"), ("result", "route")])
            .inc();
        scope
            .counter_with_labels("decision_total", [("result", "route"), ("network", "tcp")])
            .inc();
    ...
    fn gauge_decrement_saturates_at_zero() {
        let registry = MetricsRegistry::new();
        let scope = registry.scope("runtime", "task", "worker");
        let gauge = scope.gauge("active_total");
    ...
    fn recorder_gauge_decrement_saturates_at_zero() {
        let registry = MetricsRegistry::new();
        let recorder = registry.scope("runtime", "task", "worker").recorder();
        let key = metrics::Key::from_name("active_total".to_owned());
        let metadata = metrics::Metadata::new("metrics-test", metrics::Level::INFO, None);
