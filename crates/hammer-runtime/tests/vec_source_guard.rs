//! Source guard: production runtime modules must not allocate via `std::vec`.
...
#[test]
fn production_runtime_src_avoids_std_vec() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src");
    let mut violations = Vec::new();
    collect_violations(&root, &mut violations);
    assert!(
        violations.is_empty(),
        "production hammer-runtime src must use hammer_infra::vec; found:\n{}",
        violations.join("\n")
    );
}
...
fn collect_violations(dir: &Path, violations: &mut Vec<String>) {
    ...
    for entry in entries {
        ...
        // Skip pure test modules embedded as dedicated files; cfg(test) blocks in
        // production files still must not reintroduce std::vec for hot-path types.
