...
pub(crate) type Vec<T> = RawVec<T, CACHE_LINE>;
pub(crate) type Drain<'a, T> = RawDrain<'a, T, CACHE_LINE>;
pub(crate) type IntoIter<T> = RawIntoIter<T, CACHE_LINE>;

const COPY_PREFETCH_LINES: usize = 4;

pub(crate) struct RawVec<T, const ALIGN: usize = CACHE_LINE> {
    ptr: NonNull<T>,
    len: usize,
    cap: usize,
...
pub(crate) struct RawDrain<'a, T, const ALIGN: usize = CACHE_LINE> {
...
pub(crate) struct RawIntoIter<T, const ALIGN: usize = CACHE_LINE> {
    ptr: NonNull<T>,
    cap: usize,
    current: usize,
    end: usize,
