...
mod decode_tests {
    ...
    fn test_decode_all_code_joined() {
        let bytes = vec![
            ...
            0b1111_1011,
            (0b10 << 6)
                // Last two bits of 255, then six one-bits of EOS-prefix
                // padding (valid: at most 7 padding bits).
                + 0b11_1111,
        ];
...
mod error_tests {
    ...
    /// Input ending mid-code with leftover bits that are not all-ones
    /// padding reports `MissingBits` (truncation).
    #[test]
    fn truncated_symbol_is_missing_bits() {
        // "ac" is `00011 00100`; without the padding byte the last byte
        // holds `001`, which is neither a complete symbol nor padding.
        let res: Result<Vec<_>, _> = vec![0x19u8].hpack_decode().collect();
        assert!(matches!(res, Err(HuffmanDecodingError::MissingBits(_))));
    }

    /// Trailing bits that are not all-ones are malformed padding.
    #[test]
    fn malformed_padding_is_missing_bits() {
        // "ac" (10 bits) plus six trailing bits `111110`.
        let res: Result<Vec<_>, _> = vec![0x19, 0x3e].hpack_decode().collect();
        assert!(matches!(res, Err(HuffmanDecodingError::MissingBits(_))));
    }

    /// Trailing all-ones (EOS-prefix) padding is accepted.
    #[test]
    fn all_ones_padding_is_accepted() {
        // "ac" plus six padding ones.
        let res: Result<Vec<_>, _> = vec![0x19, 0x3f].hpack_decode().collect();
        assert_eq!(res, Ok(vec![b'a', b'c']));
    }

    /// An empty input has no symbols and no padding to validate.
    #[test]
    fn empty_input_succeeds_empty() {
        let res: Result<Vec<_>, _> = Vec::<u8>::new().hpack_decode().collect();
        assert_eq!(res, Ok(Vec::<u8>::new()));
    }

    /// Eight one-bits: a truncated EOS prefix, not valid padding.
    #[test]
    fn all_ones_single_byte_is_rejected() {
        let res: Result<Vec<_>, _> = vec![0xffu8].hpack_decode().collect();
        assert!(matches!(res, Err(HuffmanDecodingError::MissingBits(_))));
    }

    /// Sixteen one-bits: an EOS prefix far longer than the 7-bit maximum.
    #[test]
    fn all_ones_two_bytes_is_rejected() {
        let res: Result<Vec<_>, _> = vec![0xffu8, 0xff].hpack_decode().collect();
        assert!(matches!(res, Err(HuffmanDecodingError::MissingBits(_))));
    }

    /// Thirty-two one-bits contain the complete 30-bit EOS symbol; it must
    /// be rejected at EOF rather than treated as padding.
    #[test]
    fn complete_eos_is_rejected() {
        let res: Result<Vec<_>, _> = vec![0xffu8; 4].hpack_decode().collect();
        assert!(matches!(res, Err(HuffmanDecodingError::MissingBits(_))));
    }

    /// Every bit after the last decoded symbol must be a one-bit; a mixed
    /// suffix (`110111` after "ae") is malformed padding.
    #[test]
    fn mixed_padding_after_last_symbol_is_rejected() {
        let res: Result<Vec<_>, _> = vec![0x19u8, 0x77].hpack_decode().collect();
        assert!(matches!(res, Err(HuffmanDecodingError::MissingBits(_))));
    }

    /// An all-ones run (the EOS path) that continues with more data walks
    /// into the trie's empty `EOF` node, which resolves no symbol.
    #[test]
    fn eos_path_is_unhandled() {
        // 32 ones reach the `EOF` node; the following byte cannot resolve.
        let res: Result<Vec<_>, _> = vec![0xff, 0xff, 0xff, 0xff, 0x00].hpack_decode().collect();
        assert!(matches!(res, Err(HuffmanDecodingError::Unhandled(_, _))));
    }
}
