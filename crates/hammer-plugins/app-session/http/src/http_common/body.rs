...
mod tests {
    ...
    #[test]
    fn data_without_declared_length_is_unexpected() {
        let mut body = BodyAccumulator::from(None);
        assert_eq!(body.on_data(3), Err(BodyError::DataWithoutDeclaredLength));
    }

    #[test]
    fn empty_data_while_receiving_is_accepted_and_state_unchanged() {
        let mut body = BodyAccumulator::from(Some(3));
        assert_eq!(body.on_data(0), Ok(()));
        assert_eq!(body, BodyAccumulator::from(Some(3)));
    }

    #[test]
    fn overrun_rejects_before_mutating_state() {
        let mut body = BodyAccumulator::from(Some(4));
        assert_eq!(body.on_data(5), Err(BodyError::MoreDataThanDeclared));
        assert_eq!(body, BodyAccumulator::from(Some(4)));
    }

    #[test]
    fn chunks_count_down_to_complete_then_reject_more_data() {
        let mut body = BodyAccumulator::from(Some(7));
        assert_eq!(body.on_data(3), Ok(()));
        assert_eq!(body.on_data(4), Ok(()));
        assert_eq!(body.on_data(1), Err(BodyError::DataWithoutDeclaredLength));
    }

    #[test]
    fn fin_with_declared_incomplete_body_is_incomplete() {
        let mut body = BodyAccumulator::from(Some(3));
        assert_eq!(body.on_data(1), Ok(()));
        assert_eq!(body.finish(true), Err(BodyError::IncompleteAtEnd));
    }

    #[test]
    fn fin_without_stream_end_while_receiving_is_ok_and_state_unchanged() {
        let body = BodyAccumulator::from(Some(3));
        assert_eq!(body.finish(false), Ok(()));
        assert_eq!(body, BodyAccumulator::from(Some(3)));
    }

    #[test]
    fn fin_is_ok_when_body_complete_or_length_undeclared() {
        assert_eq!(BodyAccumulator::from(Some(0)).finish(true), Ok(()));
        assert_eq!(BodyAccumulator::from(None).finish(true), Ok(()));
        let mut consumed = BodyAccumulator::from(Some(2));
        assert_eq!(consumed.on_data(2), Ok(()));
        assert_eq!(consumed.finish(true), Ok(()));
    }
}
