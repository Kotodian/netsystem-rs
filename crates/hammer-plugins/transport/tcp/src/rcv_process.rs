use crate::{insert_tcp_session, mark_tcp_session_ready, tcp_session};
...
mod tests {
    ...
    fn install_rcv_runtime(
        ...
        let established =
            runtime
                .nodes()
                .register_internal(crate::established::TcpEstablishedNode::new(
                    handle,
                    crate::established::TcpEstablishedNext::nodes(output, drop),
                ));
    ...
    fn tcp_flags(packet: &[u8]) -> u8 {
        let tcp = crate::tcp_bytes_after_l3(packet);
        tcp[13]
    }
    ...
    fn peer_fin_in_finwait2_acks_and_enters_time_wait() {
        ...
        assert!(runtime.run_ready_nodes().expect("ack local fin") >= 1);
        {
            let queue = handle.borrow_mut().expect("tcp queue");
            assert_eq!(
                tcp_session(&*queue, session_id).expect("session").state(),
                TcpState::FinWait2
            );
        }
    ...
    fn ack_of_local_fin_in_last_ack_closes() {
        ...
        let queue = handle.borrow_mut().expect("tcp queue");
        assert!(
            tcp_session(&*queue, session_id).is_none(),
