...
#[test]
fn tcp_segment_parses_header_ports_sequence_ack_flags_window_options_and_payload() {
...
#[test]
fn tcp_segment_parses_and_writes_ns_flag() {
...
#[test]
fn tcp_segment_rejects_short_header_and_bad_data_offset() {
    assert_eq!(
...
#[test]
fn tcp_options_parse_mss_window_scale_sack_timestamp_ecn() {
...
#[test]
fn tcp_write_ack_with_sack_blocks() {
    let mut output = [0u8; 64];
...
#[test]
fn tcp_write_urgent_pointer_when_urg_set() {
    let mut output = [0u8; 64];
...
#[test]
fn tcp_non_ack_does_not_write_sack_blocks() {
    let mut output = [0u8; 64];
...
fn write_ack_for_test(output: &mut [u8], sack_blocks: &[TcpSackBlock]) -> Result<usize, TcpError> {
