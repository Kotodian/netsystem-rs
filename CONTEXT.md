...
**Hammer Stats**:
The runtime-owned observability boundary that gives named measurements a stable identity, value kind, lifecycle, ownership, and VPP-style directory and dump semantics. Live values are Hammer-owned atomics in the stats segment, and the Binary API list/dump methods are the external read surface. Hammer Stats observes domain owners; it does not become the owner of packet, node, buffer, session, or heap state.
_Avoid_: generic logging sink, packet-path registry, exporter-only facade, shared mutable state without an owner

**Direct Metric**:
A measurement whose owning domain updates a stable Stats Directory value as part of normal operation, such as a node error count or a worker-local dispatch count. Direct Metrics preserve owner-local update responsibility and do not route packet-path writes through Stats Main.
_Avoid_: sampled metric, collector-owned counter, packet-path name lookup, central hot-path map update

**Collector Metric**:
A measurement produced by a bounded observation pass over an existing owner, such as heap usage, vector rate, or buffer-pool availability. A Collector Metric has an explicit collection cadence and may be stale between collection passes.
_Avoid_: packet-path calculation, unbounded scan, direct owner replacement

**Instrumentation Observer**:
An optional measurement component that attaches to an execution or allocation seam to derive diagnostics, such as per-node buffer allocation and flow counts. An Instrumentation Observer has an explicit enablement state and lifecycle cost; VPP bufmon keeps this state private to the plugin rather than registering it in the Stats Directory.
_Avoid_: invisible callback chain, permanent tracing overhead, exporter-owned instrumentation

**Stats Directory**:
The discoverable namespace and stable storage binding for Hammer Stats entries. It manages metric identity, value kind, dimensions, and removal visibility, while each Direct Metric or Collector Metric owner remains responsible for updating its assigned values.
_Avoid_: packet-path name lookup, arbitrary string map, ownerless global counter table

**Stats Owner Registrar**:
The owner-specific registration boundary for one statistics family. A Node, Interface, Buffer, Memory, or Plugin Stats Owner declares and binds the metrics it owns through the common Hammer Stats contract; Hammer Stats does not infer ownership from a generic registration call.
_Avoid_: universal registrar with hidden owner semantics, exporter-owned registration, metric registration from packet processing

**Stats Main**:
The Main Thread Hammer Stats authority corresponding to VPP's statistics main, installed through `ThreadOwned` so directory, descriptor, and retirement structure has one serial owner. It manages Stats Directory entries, stable stats storage, collector registration and scheduling, and dump requests without owning packet, node, buffer, session, heap, or plugin domain state. Structural writes use an atomics-based `in_progress`/epoch bracket instead of VPP's structural spinlock because the Main Thread is the only structural writer.
_Avoid_: central hot-path counter owner, universal domain manager, exporter as the source of truth

**Prometheus Metric Contract**:
The common measurement vocabulary used by Hammer Stats for exposition: counters represent monotonically increasing events, gauges represent current bounded state, and histograms represent explicitly bucketed observations. VPP-specific combined or symlinked values are mapped into this contract rather than becoming new packet-path metric kinds. Prometheus attributes are definition-only: `hammer-stats` stores the descriptor (type, fq_name, help, const labels) and exposes it through the Binary API; there is no exporter and no scraping server.
_Avoid_: treating every VPP directory entry as a Prometheus type, counter reset as ordinary gauge mutation, noop histogram registration

**Bounded Metric Dimension**:
A stable, owner-lifetime dimension permitted as a Prometheus label, such as node, worker, interface, direction, heap, pool, or a plugin-declared finite category. Session, tuple, packet, buffer, and other unbounded identities are aggregated before exposition instead of becoming labels.
_Avoid_: unbounded label cardinality, dynamic object labels, packet-path label construction

**Stats Dump**:
An on-demand copy of selected current Stats Directory entries and values, matching VPP's client-side dump operation. `hammer-ipc` requests it through the existing Protobuf Binary API; the operation does not create or replace persistent data, trigger collection, enter a Worker Barrier, or scan domain-owner state.
_Avoid_: published snapshot, immutable snapshot replacement, bincode metrics protocol, separate stats transport, read-triggered collection
