...
- Route all ordinary production allocations through the Hammer main heap using `hammer-infra` data structures. SVM regions and Buffer Arena packet storage are the only allocation backends exempt from the main heap; do not add Rust-global-allocator-backed control-plane or dataplane storage.
