...
- Treat the complete tracked issue, its acceptance criteria, explicit non-goals, linked dependencies, and verification requirements as the behavior contract. Read them before editing. Do not infer extra behavior from nearby code, TODOs, comments, imagined compatibility, or a later issue.
- Keep every change traceable to a sentence in the issue. If a symbol, field, helper, test, dependency, metadata change, or abstraction cannot be justified by the issue or an existing repository contract it must be removed before review. Do not add speculative APIs, wrappers, registries, duplicate models, compatibility layers, or convenience helpers.
- Preserve existing source-of-truth modules and generated or protocol definitions. When an issue depends on an earlier issue, consume that implementation at its existing boundary; do not copy, redefine, “improve,” or silently extend it while implementing the child issue.
- Before each business-code edit, read the matching implementation in `third_party/vpp` and the closest existing Hammer implementation. Preserve ownership, lifecycle, publication order, memory layout, and visibility rules unless the issue explicitly requires a difference.
- Use the most specific installed agent for the language or domain. Agents work in a strict handoff: one implementation owner, one independent review owner, then one delivery owner. Do not duplicate another agent's search or edits, and do not start the next handoff until the prior owner reports exact changed paths, issue mapping, checks, and blockers.
- Implement test-first in vertical slices at public or explicitly approved seams. Add only tests required by the issue's stated behavior and regression risks. Do not test private implementation details or create tests for out-of-scope future behavior.
- Prefer the repository's existing types, traits, conversions, synchronization, and allocation owners. Keep visibility narrow, use static dispatch by default, use `From`/`TryFrom` at type boundaries, and use typed owner-local errors. Do not replace errors with sentinels, logs, panics, `unwrap`, `expect`, or assertions in production paths.
- Avoid one-use helper functions and abstraction layers. A private helper is justified only when it is a real reusable operation, ownership boundary, protocol abstraction, or independently testable seam. A private helper name does not authorize new behavior or a new public capability.
