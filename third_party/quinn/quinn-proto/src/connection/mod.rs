...
impl Connection {
    ...
    fn on_packet_acked(&mut self, now: Instant, packet_number: u64, info: SentPacket) -> bool {
        ...
        if info.ack_eliciting && self.path.challenge.is_none() {
            ...
            let acked = [crate::congestion::AckedPacketInfo {
                packet_number,
                bytes: info.size.into(),
                sent: info.time_sent,
                app_limited: self.app_limited,
            }];
            congestion_event_consumed = self.path.congestion.on_congestion_event_ex(
                now,
                prior_in_flight,
                &acked,
                &[],
    ...
    fn detect_lost_packets(&mut self, now: Instant, pn_space: SpaceId, due_to_ack: bool) {
        ...
        if let Some(largest_lost) = lost_packets.last().cloned() {
            ...
            if lost_ack_eliciting {
                self.stats.path.congestion_events += 1;
                let packet_event_consumed = self.path.congestion.on_congestion_event_ex(
                    now,
                    old_bytes_in_flight,
                    &[],
                    &lost_packet_infos,
