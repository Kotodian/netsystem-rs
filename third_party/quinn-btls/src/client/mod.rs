...
impl Config {
    pub fn new() -> Result<Self> {
        Self::new_with_context_config(|_| Ok(()))
    }

    /// Builds a client config and allows callers to adjust the [SslContextBuilder]
    /// before it is converted into the shared [SslContext].
    pub fn new_with_context_config<F>(configure_context: F) -> Result<Self>
    where
        F: FnOnce(&mut SslContextBuilder) -> Result<()>,
    {
    ...
    /// Sets a callback used to configure each client [Ssl] before its handshake starts.
    pub fn set_ssl_config_callback<F>(&mut self, callback: F)
    where
        F: Fn(&mut Ssl) -> Result<()> + Send + Sync + 'static,
    {
        self.ssl_config_callback = Some(Arc::new(callback));
    }

    /// Clears the per-session [Ssl] configuration callback.
    pub fn clear_ssl_config_callback(&mut self) {
...
impl Session {
    fn new(
        ...
        if let Some(configure_ssl) = &cfg.ssl_config_callback {
            configure_ssl(&mut ssl)?;
        }
