...
pub struct AmneziaMessageTypeRange {
    pub min: u32,
    pub max: u32,
}

#[cfg(feature = "amneziawg")]
impl AmneziaMessageTypeRange {
    #[inline]
...
pub struct AmneziaConfig {
    pub h1: AmneziaMessageTypeRange,
    pub h2: AmneziaMessageTypeRange,
    pub h3: AmneziaMessageTypeRange,
    pub h4: AmneziaMessageTypeRange,
    pub s1: u16,
    pub s2: u16,
    pub s3: u16,
    pub s4: u16,
...
impl AmneziaConfig {
    #[inline]
    fn prefix_len(&self, kind: AmneziaPacketKind) -> usize {
    ...
    #[inline]
    fn header_range(&self, kind: AmneziaPacketKind) -> AmneziaMessageTypeRange {
        match kind {
            AmneziaPacketKind::HandshakeInit => self.h1,
            AmneziaPacketKind::HandshakeResponse => self.h2,
            AmneziaPacketKind::CookieReply => self.h3,
            AmneziaPacketKind::Data => self.h4,
        }
    }
}

#[cfg(feature = "amneziawg")]
fn parse_incoming_packet_amnezia<'a>(
    src: &'a [u8],
    config: &AmneziaConfig,
) -> Result<Packet<'a>, WireGuardError> {
    let checks = [
        (AmneziaPacketKind::HandshakeInit, HANDSHAKE_INIT_SZ),
        (AmneziaPacketKind::HandshakeResponse, HANDSHAKE_RESP_SZ),
        (AmneziaPacketKind::CookieReply, COOKIE_REPLY_SZ),
        (AmneziaPacketKind::Data, DATA_OVERHEAD_SZ),
    ];
    for (kind, base_len) in checks {
        ...
        let packet_type =
            u32::from_le_bytes(src[prefix_len..prefix_len + 4].try_into().unwrap());
        if !config.header_range(kind).contains(packet_type) {
            continue;
        }
        let body = &src[prefix_len..];
        return match (kind, body.len()) {
            (AmneziaPacketKind::HandshakeInit, HANDSHAKE_INIT_SZ) => {
                Ok(Packet::HandshakeInit(HandshakeInit {
                    sender_idx: u32::from_le_bytes(body[4..8].try_into().unwrap()),
                    unencrypted_ephemeral:
                        <&[u8; 32] as TryFrom<&[u8]>>::try_from(&body[8..40])
                            .expect("length already checked above"),
                    encrypted_static: &body[40..88],
                    encrypted_timestamp: &body[88..116],
                }))
            }
            (AmneziaPacketKind::HandshakeResponse, HANDSHAKE_RESP_SZ) => {
                Ok(Packet::HandshakeResponse(HandshakeResponse {
                    sender_idx: u32::from_le_bytes(body[4..8].try_into().unwrap()),
                    receiver_idx: u32::from_le_bytes(body[8..12].try_into().unwrap()),
                    unencrypted_ephemeral:
                        <&[u8; 32] as TryFrom<&[u8]>>::try_from(&body[12..44])
                            .expect("length already checked above"),
                    encrypted_nothing: &body[44..60],
                }))
            }
            (AmneziaPacketKind::CookieReply, COOKIE_REPLY_SZ) => {
                Ok(Packet::PacketCookieReply(PacketCookieReply {
                    receiver_idx: u32::from_le_bytes(body[4..8].try_into().unwrap()),
                    nonce: &body[8..32],
                    encrypted_cookie: &body[32..64],
                }))
            }
            (AmneziaPacketKind::Data, DATA_OVERHEAD_SZ..=usize::MAX) => {
                Ok(Packet::PacketData(PacketData {
                    receiver_idx: u32::from_le_bytes(body[4..8].try_into().unwrap()),
                    counter: u64::from_le_bytes(body[8..16].try_into().unwrap()),
                    encrypted_encapsulated_packet: &body[16..],
                }))
            }
...
impl Tunn {
    ...
    #[inline(always)]
    pub fn parse_incoming_packet_with_amnezia<'a>(
        src: &'a [u8],
        #[cfg(feature = "amneziawg")] amnezia: Option<&AmneziaConfig>,
        #[cfg(not(feature = "amneziawg"))] _amnezia: Option<&()>,
    ) -> Result<Packet<'a>, WireGuardError> {
        if src.len() < 4 {
        ...
        #[cfg(feature = "amneziawg")]
        if let Some(config) = amnezia {
            return parse_incoming_packet_amnezia(src, config);
        }
