...
use super::{AmneziaConfig, AmneziaPacketKind};
...
impl Handshake {
    #[cfg(feature = "amneziawg")]
    fn amnezia_message_type(&self, kind: AmneziaPacketKind, fallback: u32) -> u32 {
        self.amnezia
            .map(|config| match kind {
                AmneziaPacketKind::HandshakeInit => config.h1.min,
                AmneziaPacketKind::HandshakeResponse => config.h2.min,
                AmneziaPacketKind::CookieReply => config.h3.min,
                AmneziaPacketKind::Data => config.h4.min,
            })
            .unwrap_or(fallback)
    }
    ...
    pub(super) fn format_handshake_initiation<'a>(
        ...
        #[cfg(feature = "amneziawg")]
        let handshake_init_type =
            self.amnezia_message_type(AmneziaPacketKind::HandshakeInit, super::HANDSHAKE_INIT);
        #[cfg(not(feature = "amneziawg"))]
        let handshake_init_type = super::HANDSHAKE_INIT;
        message_type.copy_from_slice(&handshake_init_type.to_le_bytes());
        // msg.sender_index = little_endian(initiator.sender_index)
    ...
    fn format_handshake_response<'a>(
        ...
        // msg.reserved_zero = { 0, 0, 0 }
        #[cfg(feature = "amneziawg")]
        let handshake_resp_type =
            self.amnezia_message_type(AmneziaPacketKind::HandshakeResponse, super::HANDSHAKE_RESP);
        #[cfg(not(feature = "amneziawg"))]
        let handshake_resp_type = super::HANDSHAKE_RESP;
        message_type.copy_from_slice(&handshake_resp_type.to_le_bytes());
