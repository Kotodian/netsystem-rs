...
// SSL_reality_client_hello_cb is called after the client has constructed a
// TLS ClientHello and before it is written to the transport. The callback must
// write a 32-byte legacy_session_id to |out_session_id|. |private_key| is the
// serialized X25519 private key for the corresponding client key share.
typedef int (*SSL_reality_client_hello_cb)(
    SSL *ssl, uint8_t out_session_id[SSL_MAX_SSL_SESSION_ID_LENGTH],
    const uint8_t *client_random, size_t client_random_len,
    const uint8_t *private_key, size_t private_key_len,
    const uint8_t *client_hello, size_t client_hello_len,
    size_t session_id_offset, void *arg);

// SSL_set_reality_client_hello_callback installs a callback that rewrites the
// client legacy_session_id in the first ClientHello. This is intended for
// REALITY-compatible clients that derive the session ID from the ClientHello
// transcript and X25519 key share.
OPENSSL_EXPORT void SSL_set_reality_client_hello_callback(
