...
static bool ssl_get_reality_client_private_key(SSL_HANDSHAKE *hs,
                                               Array<uint8_t> *out) {
  for (const auto &key_share : hs->key_shares) {
    ...
    if (!CBB_init(cbb.get(), 32) ||
        ...
        !CBBFinishArray(cbb.get(), out) || out->size() != 32) {
...
static bool ssl_apply_reality_client_hello_callback(SSL_HANDSHAKE *hs,
                                                    Array<uint8_t> *msg) {
  SSL *const ssl = hs->ssl;
  if (ssl->reality_client_hello_callback == nullptr) {
    return true;
  }

  static constexpr size_t kSessionIDOffset =
      SSL3_HM_HEADER_LENGTH + 2 /* legacy_version */ + SSL3_RANDOM_SIZE +
      1 /* legacy_session_id length */;
  if (SSL_is_dtls(ssl) || ssl->s3->initial_handshake_complete ||
      hs->session_id.size() != SSL_MAX_SSL_SESSION_ID_LENGTH ||
      msg->size() < kSessionIDOffset + SSL_MAX_SSL_SESSION_ID_LENGTH) {
    OPENSSL_PUT_ERROR(SSL, SSL_R_BAD_DATA_RETURNED_BY_CALLBACK);
    return false;
  }

  Array<uint8_t> private_key;
  if (!ssl_get_reality_client_private_key(hs, &private_key)) {
    return false;
  }

  uint8_t session_id[SSL_MAX_SSL_SESSION_ID_LENGTH];
  if (!ssl->reality_client_hello_callback(
          ssl, session_id, ssl->s3->client_random, SSL3_RANDOM_SIZE,
          private_key.data(), private_key.size(), msg->data(), msg->size(),
          kSessionIDOffset, ssl->reality_client_hello_arg)) {
    OPENSSL_PUT_ERROR(SSL, SSL_R_BAD_DATA_RETURNED_BY_CALLBACK);
    return false;
  }

  OPENSSL_memcpy(hs->session_id.data(), session_id, sizeof(session_id));
  OPENSSL_memcpy(msg->data() + kSessionIDOffset, session_id,
                 sizeof(session_id));
  return true;
}
...
bool ssl_add_client_hello(SSL_HANDSHAKE *hs) {
  ...
  if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_CLIENT_HELLO) ||
      ...
      !ssl_apply_reality_client_hello_callback(hs, &msg)) {
    return false;
  }
