...
4. Before the final test gate, use focused compile checks while developing. At the final
   ...
   Do not run tests during unfinished implementation when repository policy forbids it.
5. Commit immediately after a passing final gate. Push with an explicit upstream branch.
   Create the PR against the requested base, including behavior summary, affected crates,
...
6. Inspect PR checks. Merge only when the user authorized merging and the repository's
   ...
   checks, state that fact instead of implying CI passed.
7. After merge, verify the merge commit and base branch state. Delete the local feature
   branch only when it is no longer needed and the user requested cleanup. Run destructive
   cleanup such as `cargo clean` only when explicitly requested, and report what was removed.
...
- Do not force-push, rewrite history, close issues, or merge a PR without explicit user
  authorization. A PR can be created without merging when merge authority is absent.
...
- If checks fail, keep the branch unmerged, fix the issue, and rerun the final gate. If no
  checks are configured, distinguish local verification from remote CI.
